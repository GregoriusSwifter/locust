...
from .tokens import *
from .utils import UnicodeCommon
...
reserved = {
    ...
    'iface': KeywordToken('iface'),
    'from': KeywordToken('from'),
    'func': KeywordToken('func'),
    'proc': KeywordToken('proc'),
    'tell': KeywordToken('tell'),
    'then': KeywordToken('then'),
    'reply': KeywordToken('reply'),
    'while': KeywordToken('while'),
    'import': KeywordToken('import'),
    'return': KeywordToken('return'),
    
    # Types
    'int8': KeywordToken('int8'),
    'int16': KeywordToken('int16'),
    'int32': KeywordToken('int32'),
    'int64': KeywordToken('int64'),
    'int128': KeywordToken('int128'),
    'int': KeywordToken('int64'),
    
    'uint8': KeywordToken('uint8'),
    'uint16': KeywordToken('uint16'),
    'uint32': KeywordToken('uint32'),
    'uint64': KeywordToken('uint64'),
    'uint128': KeywordToken('uint128'),
    'uint': KeywordToken('uint64'),
    
    'float32': KeywordToken('float32'),
    'float64': KeywordToken('float64'),
    'float128': KeywordToken('float128'),
    'float': KeywordToken('float64'),
    
    'bits': KeywordToken('bits'),
    'func': KeywordToken('func'),
    'proc': KeywordToken('proc'),
    'arr': KeywordToken('arr'),
    'act': KeywordToken('act'),
    'vec': KeywordToken('vec'),
    'str': KeywordToken('str'),
    'set': KeywordToken('set'),
    'map': KeywordToken('map'),
    'ref': KeywordToken('ref'),
    'obj': KeywordToken('obj'),
    'nix': KeywordToken('nix'),
}
...
symbols = {
    ...
    ',': SymbolToken(','),
    '.': SymbolToken('.'),
    '=': SymbolToken('=', associativity='right', precedence=2),
    '+': SymbolToken('+', associativity='left', precedence=3),
    '-': SymbolToken('-', associativity='left', precedence=3),
    '*': SymbolToken('*', associativity='left', precedence=4),
    '/': SymbolToken('/', associativity='left', precedence=4),
    '^': SymbolToken('^'),
    '%': SymbolToken('%'),
    ':': SymbolToken(':'),
    '>': SymbolToken('>', associativity='left', precedence=5),
    '<': SymbolToken('<', associativity='left', precedence=5),
}
...
class Lexer(object):
    ...
    def lex_new_line(self, f):
        ...
        while True:
            ch = f.peek()
            if ch is None:
                #end of file. Make sure dedents are passed back first.
                ...
                if diff == 0:
                    ...
                    print("#%% new indent level: %s\n#%% diff is: %s" % (indent_count, diff))
                    self.cur_indent = indent_count
                    if diff > 0:
